...
The CLI sets console output and native-pipeline encoding to UTF-8 without a BOM so non-ASCII response text displays correctly on Windows PowerShell 5.1 and PowerShell 7. PowerShell pipelines still carry Unicode strings: `| Out-File` uses that cmdlet's own default encoding (UTF-16LE on Windows PowerShell 5.1, UTF-8 on PowerShell 7). Use `| Out-File -Encoding utf8` when you specifically need a UTF-8 file.
